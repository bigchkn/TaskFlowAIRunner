...
impl TaskFlowAdapter {
    ...
    pub fn execute_start(&self, task_id: &str, agent: &str) -> Result<()> {
        let mut cmd = self.taskflow_cmd();
        cmd.arg("execute").arg("start").arg(task_id).arg("--agent").arg(agent);

        let result = execute_with_timeout(cmd, Duration::from_secs(30), None)?;

        if !result.success() {
            return Err(anyhow!("taskflow-ai execute start failed: {}", result.stderr));
        }
    ...
    pub fn execute_complete(&self, task_id: &str, outcome: &str, log: &str) -> Result<()> {
        ...
        if !result.success() {
            return Err(anyhow!("taskflow-ai execute complete failed: {}", result.stderr));
        }
...
Suggested Action:
  taskflow-ai execute start TF-8"#;
        ...
        assert_eq!(task.title, "Implement the TaskFlow adapter for next/start/validate/complete/sync");

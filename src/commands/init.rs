...
use crate::config::{Config, ProviderConfig, PromptArgMode, save_config};
...
pub fn execute() -> Result<()> {
    ...
    if !Path::new(".git").exists() {
        return Err(anyhow!("This command must be run from the root of a Git repository."));
    }
    ...
    if detected_providers.is_empty() {
        ...
        for id in &detected_providers {
            providers.insert(id.to_string(), ProviderConfig {
